...
| Shows a cell in the magazine well | `ViewModelElements` / `WorldModelElements` `active` toggles |
...
the attachment cache never reaches. So the cell (`plasma_cell`) is declared inactive in the M16A4's
own `VElements`/`WElements`, anchored to its magazine bone, and this attachment switches it on.
Fitting it to another weapon means copying that element row across.
